...
-- 先移除舊政策以便重複執行
drop policy if exists markers_select_self on public.markers;
drop policy if exists markers_select_admin on public.markers;
drop policy if exists markers_insert_self on public.markers;
drop policy if exists markers_update_self on public.markers;

create policy markers_select_self on public.markers
  for select using (user_id = auth.uid());

create policy markers_select_admin on public.markers
  for select using (
    exists (
      select 1 from public.profiles p
      where p.id = auth.uid() and p.is_admin = true
...
create policy markers_insert_self on public.markers
  for insert with check (user_id = auth.uid() and email = auth.email());

create policy markers_update_self on public.markers
  for update using (user_id = auth.uid());

create index if not exists idx_markers_user_id on public.markers(user_id);
create index if not exists idx_markers_updated_at on public.markers(updated_at desc);
